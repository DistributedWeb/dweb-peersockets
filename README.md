# peersockets
[![Build Status](https://travis-ci.com/distributedweb/peersockets.svg?token=WgJmQm3Kc6qzq1pzYrkx&branch=master)](https://travis-ci.com/distributedweb/peersockets)

__Warning__: Extension messages are currently not MAC'd, so we can't detect tampering. This is still experimental!

Peersockets is a lightweight wrapper around [`@dwebstore/networker`](https://github.com/distributedweb/dwebstore-swarm-networking) that lets you exchange topic-tagged messages with peers over [`ddatabase-protocol`](https://github.com/distributedweb/ddatabase-protocol) streams.
...
### Upgrading
If you've already been using peersockets v0, take a look at [`UPGRADE.md`](https://github.com/distributedweb/peersockets/blob/master/upgrade.md) to see what's changed in v1.
...
```
### Example
This example assumes you already have Networker ([`@dwebstore/networker`](https://github.com/distributedweb/dwebstore-swarm-networking)) instance.
```
...
* `networker` is an instance of [`@dwebstore/networker`](https://github.com/distributedweb/dwebstore-swarm-networking)).
...
`peer` is a `@dwebstore/networker` Peer instance that can be obtained from `networker.peers`.
